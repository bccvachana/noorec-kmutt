export const mockData1 = {
  createdAt: {
    seconds: 1591392074,
    nanoseconds: 96000000,
  },
  lastName: "จงรุจิภิญโญ",
  firstName: "วจนะ",
  profileImgUrl:
    "https://firebasestorage.googleapis.com/v0/b/noorec-kmutt.appspot.com/o/profileImg%2F5fbc3e96-2f82-4eea-8d75-71a9adfe179d?alt=media&token=dd3f4960-1995-4f6e-8ad2-ded12ecacb7a",
  updatedAt: {
    seconds: 1591392074,
    nanoseconds: 96000000,
  },
  record: [
    {
      height: "148",
      bloodPressureHigh: "131",
      temperature: "37.3",
      weight: "94.4",
      bloodPressureLow: "90",
      rate: "103",
      bmi: "43.10",
      oxygen: "98",
      createdAt: {
        seconds: 1589639301,
        nanoseconds: 739000000,
      },
    },
    {
      height: "180",
      weight: "95.1",
      bmi: "29.35",
      createdAt: {
        seconds: 1589639349,
        nanoseconds: 875000000,
      },
    },
    {
      createdAt: {
        seconds: 1589639376,
        nanoseconds: 864000000,
      },
      temperature: "37.5",
    },
    {
      weight: "94.1",
      bloodPressureLow: "97",
      rate: "93",
      bmi: "30.73",
      oxygen: "97",
      createdAt: {
        seconds: 1589649537,
        nanoseconds: 857000000,
      },
      height: "175",
      bloodPressureHigh: "155",
      temperature: "36.0",
    },
    {
      weight: "94.8",
      bmi: "28.00",
      createdAt: {
        seconds: 1589649584,
        nanoseconds: 803000000,
      },
      height: "184",
    },
    {
      weight: "94.9",
      bloodPressureLow: "107",
      rate: "89",
      bmi: "28.97",
      oxygen: "97",
      createdAt: {
        seconds: 1589654999,
        nanoseconds: 860000000,
      },
      height: "181",
      bloodPressureHigh: "123",
      temperature: "36.0",
    },
  ],
};

export const mockData2 = {
  createdAt: {
    seconds: 1591392074,
    nanoseconds: 96000000,
  },
  lastName: "นนทวงศ์",
  firstName: "เกวลี",
  profileImgUrl:
    "https://firebasestorage.googleapis.com/v0/b/noorec-kmutt.appspot.com/o/profileImg%2F5fbc3e96-2f82-4eea-8d75-71a9adfe179d?alt=media&token=dd3f4960-1995-4f6e-8ad2-ded12ecacb7a",
  updatedAt: {
    seconds: 1591392074,
    nanoseconds: 96000000,
  },
  record: [
    {
      oxygen: "97",
      createdAt: {
        seconds: 1589639137,
        nanoseconds: 991000000,
      },
      height: "180",
      bloodPressureHigh: "131",
      temperature: "37.3",
      weight: "94.9",
      bloodPressureLow: "90",
      rate: "103",
      bmi: "29.29",
    },
    {
      weight: "94.8",
      bmi: "28.94",
      createdAt: {
        seconds: 1589639503,
        nanoseconds: 297000000,
      },
      height: "181",
    },
    {
      oxygen: "97",
      createdAt: {
        seconds: 1589649427,
        nanoseconds: 919000000,
      },
      height: "174",
      bloodPressureHigh: "155",
      temperature: "36.1",
      weight: "95.5",
      bloodPressureLow: "97",
      rate: "96",
      bmi: "31.54",
    },
    {
      weight: "95.0",
      bmi: "29.00",
      createdAt: {
        seconds: 1589649629,
        nanoseconds: 757000000,
      },
      height: "181",
    },
    {
      bloodPressureHigh: "123",
      temperature: "35.2",
      weight: "95.0",
      bloodPressureLow: "107",
      rate: "84",
      bmi: "29.32",
      oxygen: "90",
      createdAt: {
        seconds: 1589654867,
        nanoseconds: 512000000,
      },
      height: "180",
    },
    {
      temperature: "36.3",
      weight: "94.7",
      bloodPressureLow: "107",
      rate: "101",
      bmi: "29.56",
      oxygen: "97",
      createdAt: {
        seconds: 1589785555,
        nanoseconds: 774000000,
      },
      height: "179",
      bloodPressureHigh: "123",
    },
    {
      height: "180",
      weight: "94.8",
      bmi: "29.26",
      createdAt: {
        seconds: 1589785660,
        nanoseconds: 482000000,
      },
    },
    {
      createdAt: {
        seconds: 1589785714,
        nanoseconds: 471000000,
      },
      temperature: "36.7",
    },
    {
      bloodPressureHigh: "123",
      createdAt: {
        seconds: 1589785780,
        nanoseconds: 480000000,
      },
      bloodPressureLow: "107",
    },
    {
      oxygen: "98",
      createdAt: {
        seconds: 1589787412,
        nanoseconds: 681000000,
      },
      height: "180",
      bloodPressureHigh: "123",
      temperature: "35.2",
      weight: "95.0",
      bloodPressureLow: "107",
      rate: "80",
      bmi: "29.32",
    },
  ],
};

export const mockData3 = {
  createdAt: {
    seconds: 1591392074,
    nanoseconds: 96000000,
  },
  lastName: "จงรุจิภิญโญ",
  firstName: "วจนะ",
  profileImgUrl:
    "https://firebasestorage.googleapis.com/v0/b/noorec-kmutt.appspot.com/o/profileImg%2F5fbc3e96-2f82-4eea-8d75-71a9adfe179d?alt=media&token=dd3f4960-1995-4f6e-8ad2-ded12ecacb7a",
  updatedAt: {
    seconds: 1591392074,
    nanoseconds: 96000000,
  },
  record: [
    {
      bloodPressureHigh: "123",
      createdAt: {
        seconds: 1589785780,
        nanoseconds: 480000000,
      },
      bloodPressureLow: "107",
    },
    {
      oxygen: "100",
      createdAt: {
        seconds: 1589787412,
        nanoseconds: 681000000,
      },
      height: "200",
      bloodPressureHigh: "100",
      temperature: "30.0",
      weight: "100.0",
      bloodPressureLow: "100",
      rate: "100",
      bmi: "30.00",
    },
  ],
};

export const articles1 = [
  {
    normalBloodPressure: true,
    content: {
      ops: [
        {
          insert: "สวัสดีครับ อันนี้ทดลองการเลือกรูป",
        },
        {
          attributes: {
            list: "ordered",
          },
          insert: "\n",
        },
        {
          insert: "ฟกฟหกฟก",
        },
        {
          attributes: {
            list: "ordered",
          },
          insert: "\n",
        },
        {
          insert: "กฟหกฟหกฟห",
        },
        {
          attributes: {
            list: "ordered",
          },
          insert: "\n",
        },
        {
          insert: "\n",
        },
        {
          insert: {
            image: "https://i.ibb.co/VHJFzMq/d27a855ce6e0.jpg",
          },
        },
        {
          insert: "\n",
        },
        {
          insert: {
            image: "https://i.ibb.co/qshb4vb/c53d72eca8d3.jpg",
          },
        },
        {
          insert: "\n",
        },
      ],
    },
    overWeightBmi: true,
    createdAt: {
      seconds: 1591823259,
      nanoseconds: 7000000,
    },
    title: "สวัสดีครับ อันนี้ทดลองการเลือกรูป",
    lowBloodPressure: true,
    normalBmi: true,
    obaseBmi: true,
    overBloodPressure: true,
    updatedAt: {
      seconds: 1591823259,
      nanoseconds: 7000000,
    },
    extremeObaseBmi: true,
    image: "https://i.ibb.co/VHJFzMq/d27a855ce6e0.jpg",
  },
  {
    allBmi: true,
    updatedAt: {
      seconds: 1591821594,
      nanoseconds: 245000000,
    },
    allBloodPressure: true,
    content: {
      ops: [
        {
          insert: {
            image:
              "https://sites.google.com/site/primarycarecluster2017/_/rsrc/1515468639556/content-pcc/1/26230361_10156107223772028_5082792694783410370_n.jpg",
          },
        },
        {
          attributes: {
            align: "center",
          },
          insert: "\n",
        },
        {
          insert: "อาหารกับมะเร็ง",
        },
        {
          attributes: {
            align: "center",
          },
          insert: "\n",
        },
        {
          insert:
            "อาหารมีส่วนเกี่ยวข้องกับโรคมะเร็ง อาหารที่ไม่ดีหรือไม่เหมาะสมอาจเพิ่มความเสี่ยงต่อการเกิดโรคมะเร็ง ในทางตรงกันข้าม อาหารที่ดีจะสามารถป้องกันการเกิดโรคมะเร็งได้ อาหารที่ดีคืออาหารที่ถูกสุขลักษณะ มีคุณภาพ และปริมาณที่เหมาะสม รวมทั้งชะลอการดำเนินของโรคมะเร็งได้ และเสริมสร้างร่างกายให้ฟื้นตัวได้อย่างรวดเร็ว",
        },
        {
          attributes: {
            blockquote: true,
          },
          insert: "\n",
        },
        {
          insert: "เคล็ดลับการป้องกันมะเร็ง",
        },
        {
          attributes: {
            align: "center",
          },
          insert: "\n",
        },
        {
          insert: "1.รักษาสุขภาพให้แข็งแรง ดูแลน้ำหนักให้อยู่ในเกณฑ์เหมาะสม",
        },
        {
          attributes: {
            blockquote: true,
          },
          insert: "\n",
        },
        {
          insert:
            "2.กินผัก ผลไม้ให้ได้ครึ่งของอาหารแต่ละมื้อ โดยเลือกกินผัก ผลไม้หลากสี",
        },
        {
          attributes: {
            blockquote: true,
          },
          insert: "\n",
        },
        {
          insert: "3.จำกัดประเภทเนื้อสัตว์ที่มีสีแดง",
        },
        {
          attributes: {
            blockquote: true,
          },
          insert: "\n",
        },
        {
          insert:
            "4.เลือกกินอาหารที่มีประโยชน์ เน้นอาหารอ่อนเค็ม หวานน้อย และไขมันต่ำ",
        },
        {
          attributes: {
            blockquote: true,
          },
          insert: "\n",
        },
        {
          insert: "5.หลีกเลี่ยงอาหารที่ไหม้เกรียม",
        },
        {
          attributes: {
            blockquote: true,
          },
          insert: "\n",
        },
        {
          insert: "6.เน้นกินอาหารที่เตรียมและเก็บรักษาอย่างถูกสุขลักษณะ",
        },
        {
          attributes: {
            blockquote: true,
          },
          insert: "\n",
        },
        {
          attributes: {
            background: "transparent",
          },
          insert: "7.งดเครื่องดื่มที่มีแอลกอฮอล์ งดการสูบบุหรี่",
        },
        {
          attributes: {
            blockquote: true,
          },
          insert: "\n",
        },
        {
          attributes: {
            background: "transparent",
          },
          insert:
            "เรื่องของสุขภาพ ทุกคนจะต้องดูแลตนเองเป็นหลัก และปฏิบัติตัวเพื่อป้องกันโรคภัยไข้เจ็บอย่างสม่ำเสมอ ",
        },
        {
          attributes: {
            blockquote: true,
          },
          insert: "\n",
        },
        {
          attributes: {
            background: "transparent",
            color: "#073763",
            bold: true,
          },
          insert: "[ที่มาบทความ : facebook fanpageคลินิกหมอชาวบ้าน]",
        },
        {
          attributes: {
            blockquote: true,
          },
          insert: "\n",
        },
        {
          insert: "\n",
        },
      ],
    },
    createdAt: {
      seconds: 1591821594,
      nanoseconds: 245000000,
    },
    title: "อาหารกับมะเร็ง",
    image:
      "https://sites.google.com/site/primarycarecluster2017/_/rsrc/1515468639556/content-pcc/1/26230361_10156107223772028_5082792694783410370_n.jpg",
  },
  {
    normalBmi: true,
    overBloodPressure: true,
    underWeightBmi: true,
    updatedAt: {
      seconds: 1591820643,
      nanoseconds: 402000000,
    },
    extremeObaseBmi: true,
    normalBloodPressure: true,
    content: {
      ops: [
        {
          attributes: {
            alt: "กกกกกกกกา - YouTube",
          },
          insert: {
            image: "https://i.ytimg.com/vi/MgPKYpHp8qw/hqdefault.jpg",
          },
        },
        {
          insert: "\n",
        },
      ],
    },
    overWeightBmi: true,
    createdAt: {
      seconds: 1591820643,
      nanoseconds: 402000000,
    },
    title: "ไม่รู้เมื่อไหร่เธอจะได้ตอบมันสักที",
    lowBloodPressure: true,
    image: "https://i.ytimg.com/vi/MgPKYpHp8qw/hqdefault.jpg",
  },
  {
    allBmi: true,
    updatedAt: {
      seconds: 1591820256,
      nanoseconds: 826000000,
    },
    allBloodPressure: true,
    content: {
      ops: [
        {
          insert: "สวัสดีครับวันนี้เราจะมานำเสนอวิธีดูแลสุขภาพ",
        },
        {
          attributes: {
            align: "center",
            header: 2,
          },
          insert: "\n",
        },
        {
          insert: "ขอหปแอปแอปแอปแอปแอปแอปแอปแอปแอปอปแ",
        },
        {
          attributes: {
            align: "center",
          },
          insert: "\n",
        },
        {
          insert: {
            image: "https://i.ibb.co/DR38ZDn/ec340d040c78.png",
          },
        },
        {
          attributes: {
            align: "center",
          },
          insert: "\n",
        },
      ],
    },
    createdAt: {
      seconds: 1591820256,
      nanoseconds: 826000000,
    },
    title: "สวัสดีครับวันนี้",
    image: "https://i.ibb.co/DR38ZDn/ec340d040c78.png",
  },
  {
    updatedAt: {
      seconds: 1591819926,
      nanoseconds: 194000000,
    },
    normalBloodPressure: true,
    content: {
      ops: [
        {
          insert: "เหนื่อยมากเลยยยยยย",
        },
        {
          attributes: {
            header: 1,
          },
          insert: "\n",
        },
        {
          insert: "\n",
        },
        {
          insert: {
            image: "https://i.ibb.co/kcgFWcj/1b2057d86dc2.png",
          },
        },
        {
          insert: "\n",
        },
      ],
    },
    createdAt: {
      seconds: 1591819926,
      nanoseconds: 194000000,
    },
    title: "บทความแรก",
    allBmi: true,
    overBloodPressure: true,
    image: "https://i.ibb.co/kcgFWcj/1b2057d86dc2.png",
  },
];

export const articles2 = [
  {
    content: {
      ops: [
        {
          insert: "BP ALL\n",
        },
      ],
    },
    updatedAt: {
      seconds: 1591824274,
      nanoseconds: 959000000,
    },
    createdAt: {
      seconds: 1591824274,
      nanoseconds: 959000000,
    },
    title: "BP ALL",
    allBloodPressure: true,
    image: null,
  },
  {
    allBmi: true,
    content: {
      ops: [
        {
          insert: "BMI ALL\n",
        },
      ],
    },
    updatedAt: {
      seconds: 1591824260,
      nanoseconds: 962000000,
    },
    createdAt: {
      seconds: 1591824260,
      nanoseconds: 962000000,
    },
    title: "BMI All",
    image: null,
  },
  {
    content: {
      ops: [
        {
          insert: "BloodPressure : LOW",
        },
        {
          attributes: {
            header: 1,
          },
          insert: "\n",
        },
      ],
    },
    updatedAt: {
      seconds: 1591824229,
      nanoseconds: 478000000,
    },
    createdAt: {
      seconds: 1591824229,
      nanoseconds: 478000000,
    },
    title: "LOW",
    lowBloodPressure: true,
    image: null,
  },
  {
    updatedAt: {
      seconds: 1591824209,
      nanoseconds: 308000000,
    },
    normalBloodPressure: true,
    content: {
      ops: [
        {
          insert: "BMI : normal",
        },
        {
          attributes: {
            header: 1,
          },
          insert: "\n",
        },
        {
          insert: "BP : normal",
        },
        {
          attributes: {
            header: 1,
          },
          insert: "\n",
        },
      ],
    },
    createdAt: {
      seconds: 1591824209,
      nanoseconds: 308000000,
    },
    title: "normal normal",
    normalBmi: true,
    image: null,
  },
  {
    obaseBmi: true,
    updatedAt: {
      seconds: 1591824168,
      nanoseconds: 169000000,
    },
    content: {
      ops: [
        {
          insert: "BMI : Obase",
        },
        {
          attributes: {
            header: 1,
          },
          insert: "\n",
        },
        {
          insert: "BloodPressure : High",
        },
        {
          attributes: {
            header: 1,
          },
          insert: "\n\n",
        },
        {
          insert: "\n",
        },
      ],
    },
    createdAt: {
      seconds: 1591824168,
      nanoseconds: 169000000,
    },
    title: "obase & high",
    highBloodPressure: true,
    image: null,
  },
  {
    content: {
      ops: [
        {
          insert: "BMI : underWeight",
        },
        {
          attributes: {
            header: 1,
          },
          insert: "\n",
        },
      ],
    },
    underWeightBmi: true,
    updatedAt: {
      seconds: 1591824102,
      nanoseconds: 644000000,
    },
    createdAt: {
      seconds: 1591824102,
      nanoseconds: 644000000,
    },
    title: "underWeight",
    image: null,
  },
];

export const mockArticle = {
  VYNbCTdKbDeD2WuNAG39: {
    content: {
      ops: [
        {
          insert: {
            image: "https://i.ibb.co/3hJgxkZ/80c9f2d4c2dc.jpg",
          },
        },
        {
          insert: "   ",
        },
        {
          insert: {
            image: "https://i.ibb.co/wdBC42r/84d41d24e64b.jpg",
          },
        },
        {
          attributes: {
            align: "center",
          },
          insert: "\n",
        },
        {
          insert:
            "“ไข่” มักมีความสัมพันธ์กับอาหารเช้าและให้โปรตีนแก่ร่างกาย ไข่ขนาดใหญ่หนึ่งฟอง มีวิตามินและแร่ธาตุที่จำเป็น 13 ชนิดและโปรตีนคุณภาพสูงทั้งหมด 70 แคลอรี่ นอกจากนี้การวิจัยทาง",
        },
        {
          attributes: {
            link: "https://www.sanook.com/health/tag/โภชนาการ/",
          },
          insert: "โภชนาการ",
        },
        {
          insert:
            "ชี้ให้เห็นว่า “ไข่” มีบทบาทในการควบคุมน้ำหนัก ความแข็งแรงของกล้ามเนื้อ การตั้งครรภ์ที่มีคุณภาพ การทำงานของสมอง ",
        },
        {
          attributes: {
            link: "https://www.sanook.com/health/",
          },
          insert: "สุขภาพ",
        },
        {
          insert:
            "ของดวงตาและอื่น ๆ อีกมากมาย ทั้งไข่ลวก ไข่คน ไข่ตุ๋น รวมไปถึงไข่ที่ต้มสุก ทั้งหมดล้วนแล้วแต่ส่งผลดีต่อร่างกาย แต่ก็ควรกินให้ถูกจึงจะได้ประโยชน์จากไข่อย่างเต็มที่",
        },
        {
          attributes: {
            align: "justify",
          },
          insert: "\n",
        },
        {
          attributes: {
            bold: true,
          },
          insert: "ทำความเข้าใจเกี่ยวกับไข่",
        },
        {
          attributes: {
            align: "justify",
            header: 3,
          },
          insert: "\n",
        },
        {
          insert:
            "พญ. กัญจณี ธนะแพสย์ แพทย์ผู้เชี่ยวชาญด้าน อายุรศาสตร์ รพ. สมิติเวช สุขุมวิท ระบุว่า “ไข่” นั้นมีสารอาหาร 2 ชนิดที่สำคัญสำหรับสุขภาพสมอง ประกอบไปด้วย โคลีน และ ลูทีน โคลีนมีบทบาทในการพัฒนาสมองของทารกในช่วงแรกของการตั้งครรภ์ โดยเฉพาะอย่างยิ่งพื้นที่ในสมองที่ใช้สำหรับเป็นหน่วยความจำและสำหรับเรียนรู้ ในขณะเดียวกัน ลูทีนก็มีส่วนที่เกี่ยวข้องกับสุขภาพของดวงตาและมีบทบาทต่อการรับรู้ด้วยเช่นกัน อ้างอิงจากนักวิจัยของ University of Illinois แสดงให้เห็นว่าความสัมพันธ์ระหว่างลูทีนในสมองโดยวัดจากการทดสอบสายตาที่ เรียกว่า Macular Pigment Optical Density (MPOD) และการรับรู้ในเด็ก นักวิจัยพบว่า MPOD มีความสัมพันธ์เชิงบวกกับผลการเรียน",
        },
        {
          attributes: {
            align: "justify",
          },
          insert: "\n\n",
        },
        {
          attributes: {
            bold: true,
          },
          insert: "ประโยชน์ของไข่ คนเรากินไข่ได้วันละกี่ฟอง",
        },
        {
          attributes: {
            align: "justify",
            header: 3,
          },
          insert: "\n",
        },
        {
          insert:
            "ไข่อาจมีปริมาณคลอเรสเตอรอลค่อนข้างสูง แต่ไม่มีผลกระทบต่อระดับคอเลสเตอรอลในร่างกาย ไข่เป็นแหล่งโปรตีนที่มีคุณค่าทาง",
        },
        {
          attributes: {
            link: "https://www.sanook.com/health/tag/โภชนาการ/",
          },
          insert: "โภชนาการ",
        },
        {
          insert:
            "สูง มีวิตามินและแร่ธาตุต่าง ๆ มากมายรวมไปถึง วิตามินเอ วิตามิน B และ B-12 วิตามินดี ไอโอดีน โฟเลต โอเมก้า – 3  อีกทั้ง “ไข่” ยังมีราคาไม่แพง หาซื้อง่าย และวิธีที่ดีที่สุดในการกินไข่คือการต้ม การรับประทานไข่วันละหนึ่งถึงสามฟอง มีประโยชน์ต่อสุขภาพหลายอย่าง แต่จะแตกต่างกันไปในแต่ละบุคคล",
        },
        {
          attributes: {
            align: "justify",
          },
          insert: "\n",
        },
      ],
    },
    typeBloodPressure: ["low", "normal", "over", "high"],
    createdAt: 1591908871,
    title: "TestTest",
    typeBmi: ["normal", "extremeObase"],
    updatedAt: 1591908871,
    image: "https://i.ibb.co/3hJgxkZ/80c9f2d4c2dc.jpg",
  },
};

export const mockUsers = {
  b3Z0P3VX1oexxH3HlI51uuwDt6x1: {
    firstName: "เกวลี",
    profileImgUrl:
      "https://firebasestorage.googleapis.com/v0/b/noorec-kmutt.appspot.com/o/profileImg%2F7b7b826c-e806-4214-aa2c-8de2d7160b4d?alt=media&token=847cb454-bc9b-4800-a6e3-bc169a4c53ff",
    createdAt: 1591392282,
    updatedAt: 1591392282,
    lastName: "นนทวงศ์",
    record: [],
  },
  eXuA1gMp3mSN5ZShAIKfe2T8EmF3: {
    record: [],
    firstName: "พนม",
    lastName: "สุข",
    updatedAt: 1591392413,
    createdAt: 1591392413,
    profileImgUrl:
      "https://firebasestorage.googleapis.com/v0/b/noorec-kmutt.appspot.com/o/profileImg%2F52c9d7ed-ac8e-4d35-bda0-833451716a47?alt=media&token=d10d9127-a35d-4d51-a34c-d6e0c73e6198",
  },
  hm1CK8hIszbFK52CBCPqsde6okq1: {
    updatedAt: 1591392556,
    profileImgUrl: "",
    lastName: "สุขศรี",
    createdAt: 1591392556,
    firstName: "ศิวัช",
    record: [],
  },
  tAjIWxXyZ6Z7yULkxja3fKV4zwu2: {
    updatedAt: 1591863192,
    temperature: "34.1",
    temperatureCriteria: "low",
    bloodPressureCriteria: "normal",
    bloodPressureHigh: "109",
    bloodPressureLow: "72",
    oxygenCriteria: "normal",
    rateCriteria: "normal",
    oxygen: "98",
    rate: "90",
    record: [
      {
        temperature: "34.1",
        createdAt: {
          seconds: 1591862909,
          nanoseconds: 101000000,
        },
        temperatureCriteria: "low",
      },
      {
        createdAt: {
          seconds: 1591863128,
          nanoseconds: 356000000,
        },
        bloodPressureCriteria: "normal",
        bloodPressureHigh: "109",
        bloodPressureLow: "72",
      },
      {
        oxygenCriteria: "normal",
        rateCriteria: "normal",
        oxygen: "98",
        createdAt: {
          seconds: 1591863192,
          nanoseconds: 341000000,
        },
        rate: "90",
      },
    ],
    profileImgUrl:
      "https://firebasestorage.googleapis.com/v0/b/noorec-kmutt.appspot.com/o/profileImg%2F64013455-3881-4a22-b16a-36ad5c8bf122?alt=media&token=d7134a94-4c4a-41ac-968a-0000f7e87e69",
    firstName: "พนมรัตน์",
    lastName: "ประกอบสุข",
    createdAt: 1591861419,
  },
  vSKd93JvjZcMl2b87tmBskeWSjE2: {
    weight: "95.8",
    createdAt: 1591392074,
    bloodPressureHigh: "109",
    record: [
      {
        createdAt: {
          seconds: 1591864242,
          nanoseconds: 7000000,
        },
        bmiCriteria: "obase",
        weight: "95.4",
        height: "180",
        bmi: "29.44",
      },
      {
        temperatureCriteria: "normal",
        temperature: "37.0",
        createdAt: {
          seconds: 1591868783,
          nanoseconds: 538000000,
        },
      },
      {
        bloodPressureCriteria: "normal",
        bloodPressureHigh: "109",
        height: "173",
        oxygen: "98",
        rate: "112",
        bloodPressureLow: "72",
        bmi: "31.88",
        oxygenCriteria: "normal",
        createdAt: {
          seconds: 1591869043,
          nanoseconds: 592000000,
        },
        temperature: "36.9",
        bmiCriteria: "extremeObase",
        weight: "95.4",
        rateCriteria: "high",
        temperatureCriteria: "normal",
      },
      {
        createdAt: {
          seconds: 1591869261,
          nanoseconds: 259000000,
        },
        weight: "95.5",
        height: "181",
        bmi: "29.15",
        bmiCriteria: "obase",
      },
      {
        oxygen: "90",
        temperatureCriteria: "normal",
        height: "174",
        createdAt: {
          seconds: 1591871509,
          nanoseconds: 179000000,
        },
        rate: "93",
        bmi: "31.64",
        temperature: "37.1",
        rateCriteria: "normal",
        bloodPressureCriteria: "normal",
        bloodPressureLow: "72",
        weight: "95.8",
        bmiCriteria: "extremeObase",
        bloodPressureHigh: "109",
        oxygenCriteria: "low",
      },
      {
        createdAt: {
          seconds: 1591871996,
          nanoseconds: 333000000,
        },
        bmi: "60.34",
        height: "126",
        weight: "95.8",
        bmiCriteria: "extremeObase",
      },
    ],
    updatedAt: 1591871996,
    bmiCriteria: "extremeObase",
    firstName: "วจนะ",
    height: "126",
    profileImgUrl:
      "https://firebasestorage.googleapis.com/v0/b/noorec-kmutt.appspot.com/o/profileImg%2F5fbc3e96-2f82-4eea-8d75-71a9adfe179d?alt=media&token=dd3f4960-1995-4f6e-8ad2-ded12ecacb7a",
    bloodPressureCriteria: "normal",
    oxygenCriteria: "low",
    lastName: "จงรุจิภิญโญ",
    bloodPressureLow: "72",
    bmi: "60.34",
    rate: "93",
    rateCriteria: "normal",
    temperature: "37.1",
    oxygen: "90",
    temperatureCriteria: "normal",
  },
};
